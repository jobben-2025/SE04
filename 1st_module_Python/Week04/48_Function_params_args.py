...
#can be empty, is the variable in between () of function
#Argument: The actual value passed to a function when calling it.
#is actual value in between () of function

def add(a, b):
    return a + b
try:
    add(1)  # Raises TypeError due to missing argument
    #When calling a function, the number of provided arguments must match the number of required parameters 
    # (unless defaults or arbitrary arguments are used). Mismatches result in a TypeError.
except Exception as e:
    print(e)

def square(x): 
    return x * x    #A function can use the return statement to output a value, which can then be used by the 
#caller. If no return is specified, it returns None by default.
result = square(4)  # result will be 16

def greet(name, greeting): 
    print(f"{greeting}, {name}!")
greet("Reagan", "Howdy")  # Positional arguments. Prints: Howdy, Reagan! => name and greeting are switched
...
def greet(name, greeting="Hello"):      #If no argument is provided for such parameters, the default is used. 
    print(f"{greeting}, {name}!")
greet("Bob")  # Prints: Hello, Bob! using the default greeting
greet(name="Bob", greeting="Howdy")  # Prints: Howdy, Bob!

#Arbitrary Positional Parameters with *args         //arbitrary = frei wählbar
#*args collects extra positional arguments into a tuple, allowing functions to accept an arbitrary number 
# of arguments. You can of course use anything else beside *args // === tuple
...
#Arbitrary Keyword Parameters with **kwargs     // = keyword arguments
#**kwargs collects extra keyword arguments into a dictionary, enabling dynamic parameter lists with keys. 
# You can of course use anything else beside **kwargs === dictionary!
...
#Using / and * in function definitions, you can enforce parameters to be passed only positionally or 
# only by keyword, respectively.
